...
// Pop and invoke the next armed timer, returning the tick promise so the
// caller can await completion for immediate-handler ticks, or hold the
// promise for deferred-handler ticks.
...
function popAndFire(timers: FakeTimer[]): Promise<void> {
  expect(timers.length, "a tick should be armed").toBeGreaterThan(0);
  const timer = timers.shift() as FakeTimer;
  expect(timer.cancelled).toBe(false);
  return timer.callback() as unknown as Promise<void>;
}

// Minimal mock store for overlap / claim-throws tests.
function mockStore(behaviour: {
  ...
  store: {
    ...
    listMessages(): Promise<{ items: readonly MessageRecord[]; nextCursor: string | null }>;
  ...
  return {
    store: {
      ...
      async markOutboxItemSucceeded() { state.markSucceededCalls += 1; state.pendingCount = Math.max(0, state.pendingCount - 1); },
...
describe("WATS-175c outbox worker loop", () => {
  test("drains pending items until countOutboxPending reaches zero", async () => {
    ...
    try {
      for (let i = 0; i < 3; i += 1) {
        await store.enqueueOutboxItem({ id: `drain-${i}`, payloadHash: hash(`${i}`), createdAt: "2026-06-01T00:00:00.000Z" });
      }
      expect(await store.countOutboxPending()).toBe(3);

      const { scheduler, timers } = fakeScheduler();
      const reports: { processed: number; succeeded: number; failed: number; pending: number }[] = [];
  ...
  test("a failed handler schedules a retry and the item stays claimable later", async () => {
    ...
    await store.migrate();
    try {
      await store.enqueueOutboxItem({ id: "retry-1", payloadHash: hash("f"), createdAt: "2026-06-01T00:00:00.000Z" });
  ...
  test("overlapping ticks are skipped while a handler is in flight", async () => {
    const item: OutboxItem = {
      ...
      payloadHash: hash("e"), nextAttemptAt: null,
    ...
    let handlerEnteredResolve: (() => void) | null = null;
    ...
    // Tick 1: claims the item, handler enters but does not resolve.
    const tick1 = popAndFire(timers);
    await handlerEntered;
    expect(handlerCalls).toBe(1);
    expect(store.claimCalls).toBe(1);

    // Simulate a second timer firing while the first tick is in flight. It
    // must skip without claiming again.
    const beforeClaims = store.claimCalls;
    scheduler.setTimeout(() => { /* placeholder; replaced below */ }, 1);
    const overlapTimer = timers.pop() as FakeTimer;
    await overlapTimer.callback();
    expect(store.claimCalls).toBe(beforeClaims);

    // Release the in-flight handler; the tick completes and re-arms exactly
    // one next timer.
    (releaseHandler as (() => void) | null)?.();
    await tick1;
  ...
  test("stop() awaits an in-flight run before resolving", async () => {
    const item: OutboxItem = {
      ...
      payloadHash: hash("d"), nextAttemptAt: null,
      createdAt: "2026-06-01T00:00:00.000Z", updatedAt: "2026-06-01T00:00:00.000Z"
    };
    const { store } = mockStore({ items: [item] });
    const { scheduler, timers } = fakeScheduler();
