...
import { createWatsServiceApp, createWatsServiceOpenApiDocument, WatsServiceError } from "@switchbord/service";
...
const SERVE_HELP = `Usage: wats serve --config <path> --dry-run [--profile <name>] [--host <host>] [--port <port>] [--print-routes]
       wats serve --config <path> --live --yes-live [--profile <name>] [--host <host>] [--port <port>]

Start the @switchbord/service Request -> Response app as a local Bun.serve dry-run process.
...
Live guard contract (documented, still fail-closed in this build):
  --live                Declare intent to use credential-gated live service mode.
  --yes-live            Acknowledge live Graph/API side effects before live mode can proceed.
  WATS_LIVE_ENABLE=1    Environment-level live intent for operators that avoid shell history flags.
  WATS_YES_LIVE=1       Environment-level acknowledgement paired with live intent.
...
Live mode is not available until the follow-up secret-resolution slice. In this build the guard fails closed before env-secret resolution, env-file parsing, service bind, or Meta Graph calls. --env-file secret resolution is not implemented and is rejected explicitly; .env.local is never read implicitly. The dry-run process does not resolve env-secret values, read .env files, call Meta Graph APIs, or print config paths, profile names, env names, or token values.
${NO_LIVE_CREDENTIALS}
`;
...
const SERVE_LIVE_GUARD_FLAGS = new Set(["--live", "--yes-live", "--env-file"]);
const LIVE_GUARD_ERROR = "Live serve mode is gated and not available in this build. Run `wats serve --help` for usage.\n";
...
function parseServeArgs(args: readonly string[]): ServeArgs {
  for (let index = 0; index < args.length; index += 1) {
    ...
    if (SERVE_LIVE_GUARD_FLAGS.has(flagName)) {
      if (arg.includes("=") || hasFlagName(args.slice(index + 1), flagName)) return { ok: false, reason: "live_guard" };
      if (flagName === "--env-file") {
        const next = args[index + 1];
        if (next === undefined || next.startsWith("-")) return { ok: false, reason: "live_guard" };
        index += 1;
      }
      continue;
    }
    if (flagName === "--dry-run" || flagName === "--print-routes") {
  ...
  const valueArgs = args.filter((arg) => {
    const flagName = arg.includes("=") ? arg.slice(0, arg.indexOf("=")) : arg;
    return flagName !== "--dry-run" && flagName !== "--print-routes" && flagName !== "--live" && flagName !== "--yes-live" && flagName !== "--env-file";
  });
  const hasLiveIntent = hasFlagName(args, "--live") || (globalThis as { process?: { env?: Record<string, string | undefined> } }).process?.env?.WATS_LIVE_ENABLE === "1";
  const hasLiveAcknowledgement = hasFlagName(args, "--yes-live") || (globalThis as { process?: { env?: Record<string, string | undefined> } }).process?.env?.WATS_YES_LIVE === "1";
  if (hasLiveIntent || hasLiveAcknowledgement || hasFlagName(args, "--env-file")) {
    return { ok: false, reason: "live_guard" };
  }
  ...
  if (!hasFlagName(args, "--dry-run")) return { ok: false, reason: "usage" };
...
function formatServeRoutes(profile: WatsProfileConfig): string {
  return [`serve dry-run routes`, ...serveRouteLines(profile)].join("\n") + "\n";
}

function formatServeReady(host: string, port: number): string {
  return [
    "serve dry-run",
    "status: listening",
    `address: http://${host}:${port}`,
    "graph: dry-run mock transport",
    "profile: [REDACTED_PROFILE]",
    "No live credentials are read or required by this command."
  ].join("\n") + "\n";
}
...
async function serveCommand(args: readonly string[]): Promise<CliCommandResult> {
  ...
  if (!parsed.ok) {
    ...
    if (parsed.reason === "live_guard") return fail(LIVE_GUARD_ERROR);
  ...
  try {
    ...
    if (parsed.printRoutes) return ok(formatServeRoutes(profile));
