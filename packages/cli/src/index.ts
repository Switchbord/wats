...
Live Graph/API mode, --live, --yes-live, and --env-file are not implemented and fail closed. The dry-run process does not resolve env-secret values, read .env files, call Meta Graph APIs, or print config paths, profile names, env names, or token values.
${NO_LIVE_CREDENTIALS}
`;
...
const SERVE_VALUE_FLAGS = ["--config", "--profile", "--host", "--port"] as const;
const SERVE_ALLOWED_FLAGS = ["--config", "--profile", "--host", "--port", "--dry-run", "--print-routes"] as const;
const SERVE_FAIL_CLOSED_FLAGS = new Set(["--live", "--yes-live", "--env-file"]);
...
function parseServeArgs(args: readonly string[]): ServeArgs {
  for (let index = 0; index < args.length; index += 1) {
    ...
    if (SERVE_FAIL_CLOSED_FLAGS.has(flagName)) return { ok: false, reason: "usage" };
  ...
  const valueArgs = args.filter((arg) => {
    const flagName = arg.includes("=") ? arg.slice(0, arg.indexOf("=")) : arg;
    return flagName !== "--dry-run" && flagName !== "--print-routes";
  });
