...
`wats serve --dry-run` starts the local `@switchbord/service` app with synthetic in-memory secrets and a no-network Graph transport. It does not resolve env-secret values, read `.env.local`, or call Meta Graph APIs. Credential-gated live serve mode remains future work.
