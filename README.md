...
The 0.3.5 line is an alpha compatibility and public-onboarding patch release. It keeps the safe local `wats setup` wizard and 0.3.2 package-manager fix line, then adds credential-free WhatsApp/Graph compatibility deltas, refreshed platform guidance, public community governance files, credential-gated local live `wats serve` for webhook/Graph smoke testing, and the first experimental `@wats/persistence` SQLite package foundation. `wats setup` writes a safe `wats.config.yaml` with env-secret references plus an ignored `.env.local` for local secrets without making Meta calls. Live serve requires explicit `--live --yes-live --env-file .env.local`; WATS does not read `.env.local` implicitly. Docker image publication, service persistence integration/outbox worker, production hosting, token validation against Meta, and multi-profile credential editing are not included.
