...
Current release: `0.3.12-alpha-compatibility`.
...
The 0.3.12 line is an alpha compatibility and local-operator patch release. It keeps the safe local setup/live-serve flow, the experimental `@wats/persistence` SQLite package foundation, optional service idempotency injection, CLI package version/upgrade checks, and the container deployment recipe (`deploy/railway/`, see `deploy/railway/README.md`), then adds an opt-in `wats serve --paas` native PaaS mode that reads the platform-injected `$PORT` and binds `0.0.0.0` so managed platforms (Railway, Fly, Render, Cloud Run) no longer need an external entrypoint shim; `$PORT` is ignored unless `--paas` is passed, so default and local behavior is unchanged. `wats setup` writes a safe `wats.config.yaml` with env-secret references plus an ignored `.env.local` for local secrets without making Meta calls. Live serve requires explicit `--live --yes-live --env-file .env.local`; WATS does not read `.env.local` implicitly. Published Docker registry images, background outbox workers, production hosting, token validation against Meta, and multi-profile credential editing are not included.
