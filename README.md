...
Current release: `0.3.11-alpha-compatibility`.
...
The 0.3.11 line is an alpha compatibility and local-operator patch release. It keeps the safe local setup/live-serve flow, the experimental `@wats/persistence` SQLite package foundation, optional service idempotency injection, CLI package version/upgrade checks, and the container deployment recipe (`deploy/railway/`, see `deploy/railway/README.md`), then enriches the opt-in `WATS_LOG_WEBHOOK_EVENTS` observability with a PII-safe `detail` (normalized message type or status value) and adds an opt-in `WATS_ECHO_REPLY` demo auto-reply that exercises the dispatch-to-send round-trip and logs a PII-safe reply outcome (sent, or failed with the Meta error code), and adds opt-in `createReliableTransport` retry/backoff/timeout for Graph requests, and surfaces PII-safe Meta Graph error codes from service send-route 502 responses. `wats setup` writes a safe `wats.config.yaml` with env-secret references plus an ignored `.env.local` for local secrets without making Meta calls. Live serve requires explicit `--live --yes-live --env-file .env.local`; WATS does not read `.env.local` implicitly. Published Docker registry images, background outbox workers, production hosting, token validation against Meta, and multi-profile credential editing are not included.
