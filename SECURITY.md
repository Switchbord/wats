...
The current repository may still be source-only and package-private while release readiness work continues. Public vulnerability handling applies to the active repository line, not to unpublished package artifacts.

## Reporting a vulnerability

Do not open a public issue for suspected vulnerabilities, leaked credentials, bypassable webhook signature checks, token redaction failures, or private data exposure.

Until a dedicated private advisory channel is configured for `switchbord/wats`, report privately to the maintainers through the project owner or the private coordination channel that gave you repository access. Include:
...
Never include live WhatsApp access tokens, app secrets, service bearer values, WABA IDs, phone-number IDs, private webhook payloads, Linear snapshots, or customer/user data in the report. Use placeholders such as `WATS_GRAPH_ACCESS_TOKEN` and synthetic IDs.
...
Maintainers should acknowledge private reports, reproduce with synthetic or redacted data when possible, file private tracking work, and publish a public advisory or changelog note once a fix is available if disclosure is appropriate.

For fixes, prefer regression tests that prove the bypass without revealing private data. Webhook signature, secret comparison, redaction, parser, route, and Graph transport fixes should include adversarial malformed-input coverage and docs updates describing the final safe contract.
