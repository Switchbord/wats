...
@wats/testing
  workspace fixtures and workspace policy tests
```
...
Those app-layer packages may depend on `@wats/core`; lower-level packages should not. architecture notes keeps the alpha CLI/runtime/operator layer in this monorepo by default rather than splitting it into a second repository.
