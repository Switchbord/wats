...
Linear is the source of truth for issue-level planning and prioritization. Repository docs may summarize current roadmap shape, but do not add repo-local deferred ledgers or private handoff notes for backlog tracking.
...
- Do not commit secrets, access tokens, app secrets, private WABA IDs, phone-number IDs, live webhook payloads, or raw customer/user data.
...
- Do not paste private Linear snapshots, local handoff files, or maintainer-only planning artifacts into public docs.
...
Public docs pages must be listed in `docs/public-docs-manifest.json`. Maintainer-only docs and handoff material must stay out of the public manifest and should not be added to public-release history.
...
- [ ] No secrets, private operational snapshots, or generated build artifacts are committed.
