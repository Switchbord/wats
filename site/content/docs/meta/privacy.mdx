...
These endpoints are disabled by default and require the service bearer token when enabled; a missing or wrong token returns the same `404` as an unknown route. No default outbound telemetry is sent. See the [Telemetry operator guide](/docs/guides/telemetry) and the [Service Reference](/docs/reference/service) for the threat model, attribute dictionary, Prometheus scrape config, and an OpenTelemetry adapter example.
