...
| `compare_templates`, `unpause_template`, `migrate_templates` | `compareTemplates`, `unpauseTemplate`, `migrateTemplates` | shape-only | Archive/unarchive and bulk auth/library helpers remain planned. `migrateTemplates` normalizes both documented `failed_templates` map responses and pywa-style list responses. |
