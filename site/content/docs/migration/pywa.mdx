...
| pywa `tracker` | WATS `bizOpaqueCallbackData`-style inputs where supported | shape-only, partial | Check the specific WATS helper; WATS uses camelCase. |
...
| pywa template component DSL | WATS component builders such as `buildTemplateHeaderComponent` | shape-only, partial | Core HEADER/BODY/FOOTER/BUTTONS helpers exist; pywa's larger DSL is broader. |
