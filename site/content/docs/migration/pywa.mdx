...
| callback overrides, token exchange, remaining profile/commerce updates | No first-class WATS helper yet | planned | Requires explicit operator authorization when it lands. |
| QR code CRUD, block/unblock users, token exchange | Block API helpers exist (`listBlockedUsers`, `blockUsers`, `unblockUsers`); QR code CRUD exists (`createQrCode`, `listQrCodes`, `getQrCode`, `updateQrCode`, `deleteQrCode` on `PhoneNumberClient`); token exchange is not built | shape-only (Block API + QR codes); token exchange planned | |
...
- mutating admin APIs beyond profile/commerce updates, token flows, callback overrides, and phone registration/deregistration. (QR code CRUD is now shape-only; token exchange and catalog CRUD remain planned.)
