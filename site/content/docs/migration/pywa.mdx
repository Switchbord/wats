...
| pywa local file / bytes / file-like media sends | `uploadAndSendImage` / `uploadAndSendVideo` / `uploadAndSendAudio` / `uploadAndSendDocument` / `uploadAndSendSticker` for in-memory `Blob` / `ArrayBuffer` / `Uint8Array`; `uploadMedia` then send by id remains available | shape-only for in-memory bodies; filesystem path helpers planned | WATS keeps `@wats/graph` runtime-neutral, so file-path convenience needs a Node/Bun-specific wrapper. |
...
| `compare_templates`, `unpause_template` | `compareTemplates`, `unpauseTemplate` | shape-only | `migrate_templates`, archive/unarchive, and bulk auth/library helpers remain planned. |
...
- pywa sent-update waiters beyond the process-local `startChat` surface (call permission waits and broader composer helpers).
