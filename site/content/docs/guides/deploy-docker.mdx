...
<DocMeta status="shape-only — Railway Dockerfile ships; generic Compose/registry shape is planned" lastReviewed="2026-07-20" />
...
logs. `wats serve` now supports this via `--database-url-env WATS_DATABASE_URL`
(see [CLI reference](/docs/reference/cli)).

## Volumes and persistence

`wats serve` has opt-in durable persistence (WATS-204):

- `--database <sqlite-path>` opens and migrates a local SQLite file **before**
  binding, injects it into the service config (metadata and message-history
  only — no raw-message outbox or distributed queue), and closes it after
  in-flight work drains on shutdown. Relative paths resolve under the config
  directory. Without either flag the service is stateless (in-memory only).
- `--database-url-env <env-name>` resolves a PostgreSQL DSN from the named env
  var; the raw DSN never appears on the command line or in logs. Requires the
  optional `pg` package in the runtime.

For container SQLite, mount a writable data directory such as `/var/lib/wats`
and pass `--database /var/lib/wats/wats.sqlite`. For multi-replica
deployments, use `--database-url-env WATS_DATABASE_URL` with a managed
Postgres. The Railway entrypoint surfaces these as the optional
`WATS_DATABASE` and `WATS_DATABASE_URL_ENV` env vars (both off by default).
