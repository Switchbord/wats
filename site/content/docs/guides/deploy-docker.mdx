...
description: Container deployment shapes around the implemented wats serve contract — future Dockerfile and Compose patterns, current PaaS support.
---

<DocMeta status="planned — serve is implemented; container packaging is a documented shape, not a shipped artifact" lastReviewed="2026-06-12" />
...
tunnel. There is no supported root Dockerfile, no supported Compose file, no
container image release, no production hosting contract, and no
container-registry workflow today. This page documents the shape future Docker
artifacts should take around the serve contract.
