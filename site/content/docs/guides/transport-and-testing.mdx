...
<DocMeta status="shape-only — MockTransport and consumer fixtures; no live Meta calls" lastReviewed="2026-06-12" />
...
Any `Transport` is just an object with a `request` method. WATS ships `createReliableTransport` as an opt-in decorator for retries with jittered backoff, `Retry-After` handling, and per-attempt timeouts without adding a dependency or changing the default `GraphClient` behavior.
...
The decorator retries transient `GET`/`DELETE` failures (`GraphNetworkError`, HTTP `429`, and HTTP `5xx`) using exponential backoff with full jitter. It honors Meta's `Retry-After` header for `429` / `503` responses and caps every delay at `maxDelayMs`. Caller aborts are never retried and never reach the inner transport: a caller `AbortSignal` is forwarded into the rate limiter's wait, so an abort during admission rejects promptly without consuming a token or dispatching a request. Per-attempt timeouts compose with a caller-provided `AbortSignal` via native `AbortSignal.any` / `AbortSignal.timeout`, and each attempt gets the full `timeoutMs` budget (it is per-attempt, not a total deadline).

WATS does **not** retry non-idempotent `POST` / `PUT` / `PATCH` request failures by default, with one narrow exception: HTTP `429` is treated as a rate-limit response across methods. A no-response POST failure (connection reset, socket closed mid-upload, read timeout) is ambiguous: the server may have received and applied the request before the response was lost. An `Idempotency-Key` header on its own proves nothing — the client sets it, and the server has no obligation to honor it unless the endpoint documents verified remote idempotency. The `retryPosts` modes (`"network-only"`, `"always"`) opt into retry only when you have independently verified the endpoint is idempotent or dedupes by `Idempotency-Key` server-side. The key is a necessary, not sufficient, condition; it never overrides the default `"never"`. The default client applies no retries.
...
- [Client reference](/docs/reference/client) — `GraphClient` options and the request pipeline.
- [Errors reference](/docs/reference/errors) — `GraphNetworkError` and the typed error hierarchy.
