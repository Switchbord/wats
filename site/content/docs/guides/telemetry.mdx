...
WATS ships with **no default outbound telemetry**. Importing `@wats/service` and starting `createWatsServiceApp` does not contact WATS maintainers, a hosted observability vendor, or any analytics endpoint. All telemetry is opt-in and operator-controlled:

- `/metrics` is not registered unless you explicitly point a Prometheus-compatible scraper at it.
- `/status` is not advertised; a missing or wrong service bearer token returns the same `404` body as any unknown route so the endpoint does not leak its own existence.
- `/debug/diagnostics` is a bounded, redacted JSON snapshot for support bundles.
- The optional `TelemetrySink` (TELEMETRY-C) emits events only to a sink you own and wire yourself.
...
| Endpoint | Purpose | Default | Auth failure |
|---|---|---|---|
| `GET /metrics` | Prometheus/OpenMetrics exposition of runtime counters and histograms | Not registered unless enabled in config | `404` identical to catch-all 404 |
| `GET /status` | Redacted operator status snapshot | Not registered unless enabled in config | `404` identical to catch-all 404 |
| `GET /debug/diagnostics` | Bounded support snapshot with version, route inventory, and recent error classes only | Not registered unless enabled in config | `404` identical to catch-all 404 |

All three endpoints reuse the same `serviceBearerToken` as the message API routes, but they fail closed to `404` on missing or mismatched tokens. This is intentional: telemetry surface availability is more sensitive than the public message-sending routes.
...
| `http_request_duration_seconds` | `http.route`, `http.request.method`, `http.status_code`, `http.response.status.class` | sink receives the full set; `/metrics` histogram narrows to `route` + `status_class` |
