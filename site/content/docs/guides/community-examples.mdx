...
<DocMeta status="shape-only — examples are offline by default" lastReviewed="2026-06-12" />
...
- `examples/minimal-bot/` — runnable 60-second offline bot using `@wats/service`, MockTransport, and `bun run demo`.
...
- `examples/offline-bot/README.md` — docs-only walkthrough of an offline MockTransport bot with synthetic webhook payloads/envelopes (no runnable package).
- `examples/config/` — placeholder YAML and JSON config templates plus `.env.example` for local ignored copies.
