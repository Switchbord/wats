...
<DocMeta status="active" appliesTo="@wats/graph, @wats/persistence, @wats/service" lastReviewed="2026-07-05" />
...
The helper only sees inbound rows projected into `wats_messages`. The service projects outbound sends automatically; inbound projection wiring is landing separately, so until it ships the helper returns `closed` for every phone — the safe default (send a template). See the [Persistence Reference](/docs/reference/persistence) for the window state shape.
