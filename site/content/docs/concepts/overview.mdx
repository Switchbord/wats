...
App-layer packages may depend on `@wats/core`; lower-level packages must not. The dependency arrows only point down — `@wats/core` never imports `@wats/http`. `@wats/http` is an application-edge package that sits above `@wats/core` (see the [package map](/docs/concepts/package-map) for the full dependency graph).
