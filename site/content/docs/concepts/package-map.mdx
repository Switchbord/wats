...
- Purpose: experimental persistence contracts, the SQLite local adapter, and the Postgres adapter, plus the migration runner, webhook/service idempotency records, message projection, and outbox record APIs.
...
| `@wats/persistence` | Y | Y | N | N | SQLite and optional Postgres adapters are server/runtime-side |
