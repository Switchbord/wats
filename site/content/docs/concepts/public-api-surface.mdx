...
- legacy raw parser/router helpers: `parseWebhookUpdate`, `createUpdateRouter` — **deprecated**, marked `@deprecated` at the type level; superseded by `normalizeWebhookEnvelope` + `TypedRouter` and scheduled for barrel removal next minor
...
- legacy raw filter namespace and `@wats/core/filters` — **deprecated**, marked `@deprecated` at the type level; superseded by `filtersTyped` and scheduled for barrel removal next minor
- `TypedRouter`
