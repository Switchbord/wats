...
`@wats/types` ships discriminated-union shapes for every inbound WhatsApp domain object the library exposes. Narrow via `switch` on the `type` discriminator; exhaustive switches with a `never` default branch catch any future variant added to the union.
