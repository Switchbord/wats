...
The SQLite adapter is for local and single-instance testing — not a production or multi-replica recommendation.
...
The CLI has no database navigation commands yet.

## Non-goals

Not implemented yet: a Postgres adapter (it must satisfy the same root contract, keep database URLs secret, and never be required by default CI), CLI thread/message navigation, delivery/read status UI, raw webhook body storage, automatic service send enqueueing, production hosting guarantees, and live Meta validation.
