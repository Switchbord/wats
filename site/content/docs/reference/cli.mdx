...
- Shutdown is graceful and finite: `server.stop(false)` stops admitting new requests and lets in-flight work finish within a finite 10s drain window (polling the server's pending-request counters), then force-stops, then closes the store. Repeated shutdown signals are idempotent. The bin awaits this async shutdown on `SIGINT`/`SIGTERM`.
