...
Not implemented yet: production hosting/deployment wrappers, `wats config print --redacted`, `wats config paths`, and optional live checks behind explicit credential-gated flags — meaning both a live flag and an operator acknowledgement before any live check runs. Planned commands keep the no-live-credentials default.
