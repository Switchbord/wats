...
### `wats serve ... --database <sqlite-path>` / `--database-url-env <env-name>`

Opt-in durable persistence for `wats serve`. Both modes are mutually exclusive and off by default: without either flag the service is stateless (in-memory only), byte-identical to prior behavior.

```bash
# Local SQLite file (opened + migrated before binding, closed on shutdown)
wats serve --config wats.config.yaml --dry-run --database ./wats.sqlite

# PostgreSQL via a named env var (the raw DSN never appears on the command line)
WATS_DATABASE_URL=postgres://user:pass@host:5432/wats \
  wats serve --config wats.config.yaml --dry-run --database-url-env WATS_DATABASE_URL
```

- `--database <sqlite-path>` opens and migrates a local SQLite file **before** binding the port, so an open/migration failure fails closed without ever touching the network. The path is a safe local filename (no traversal, control chars, or token-like names); relative paths resolve under the config directory. `:memory:` is allowed for tests.
- `--database-url-env <env-name>` resolves a PostgreSQL connection string from the named environment variable on startup. The env name must be a safe identifier (`[A-Z][A-Z0-9_]*`); the raw DSN is never accepted on the command line and never printed. Requires the optional `pg` package in the runtime.
- The store is injected into the service config (metadata and message-history only; no raw-message outbox or distributed queue is implied) and closed **after** active work drains on shutdown.
- In live mode, env-secret values are resolved **before** the persistence store is opened, so an invalid or missing env file fails with `SecretResolutionError` without creating a SQLite file on disk. Static config/PaaS validation still runs before any write.
- Shutdown is graceful and finite in two phases: `server.stop(false)` stops admitting new requests and lets in-flight work finish within a finite 10s server-drain window (polling the server's pending-request counters), then force-stops; the store is then closed within a **separate** finite 5s timeout so a hung pg query that never settles cannot block the bin exit forever. The bin awaits this async shutdown on `SIGINT`/`SIGTERM`. Repeated shutdown calls return the same in-flight drain promise (idempotent).
