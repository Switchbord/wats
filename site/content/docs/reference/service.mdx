...
Methods are intentionally synchronous so the calling code never awaits a remote export. Attribute keys follow OpenTelemetry conventions where a standard exists:
...
| `http_request_duration_seconds` | `http.route`, `http.response.status.class` | method omitted to match histogram cardinality |
