...
| `serviceMode` | Coarse mode (`low-config`, `persistence-only`, `graph`) | safe |
| `runtime` | `"bun"` or `"node"` only | safe |
...
| `configShape` | Config field names with env-secret references (`env:VAR`) only | no values, no paths |

```bash
curl -s -H "Authorization: Bearer $WATS_...KEN" https://your-service/debug/diagnostics
```

Limits are defensive:

- No more than 10 distinct error classes are retained; when the cap is hit the lowest-count class is dropped.
