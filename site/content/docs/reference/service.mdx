...
<DocMeta status="shape-only — webhook ingress and text send live-validated; other routes shape-only" lastReviewed="2026-06-21" />

`@wats/service` exposes a runtime-neutral `Request -> Response` app composing the Graph client, webhook adapter, config, and WhatsApp facade. The CLI provides dry-run and credential-gated live `wats serve` wrappers. A Railway-targeted Dockerfile ships in the repository; see [Docker deployment](/docs/guides/deploy-docker). Hosting, TLS, and storage volumes remain operator-owned.
...
| `${profile.service.apiPrefix}/conversations/{phone}/window` | GET | service bearer | Returns the 24-hour customer-service window state for a phone number when a `PersistenceStore` is injected. A missing or invalid token returns `401` (uniform `/api/*` posture, matching `/api/messages`). Returns `503 persistence_not_configured` when no store is injected, or `503 persistence_unavailable` when the store throws. |
...
`/api/*` operator routes (`/api/messages`, `/api/messages/{messageId}`, `/api/conversations/{phone}/window`) return `401 unauthorized` on a missing or invalid bearer token. This is dev-friendly: a caller can tell auth failure from a wrong path.

Telemetry endpoints (`/metrics`, `/status`, `/debug/diagnostics`) keep the existence-hiding posture. A missing or invalid token returns `404`, byte-identical to the catch-all, so their existence is not disclosed to unauthenticated callers.
...
`/healthz` reports process liveness. `/readyz` returns `200 { ok, service }` when no store is configured or the configured store is healthy and at the current schema version. A store failure or schema mismatch returns `503 persistence_unavailable`. Neither endpoint requires auth or calls Meta.

`/status` requires the service bearer token and returns operator data. Missing or invalid auth returns the catch-all `404`. The snapshot omits tokens, phone numbers, WABA ids, paths, raw webhook bodies, message text, and stack traces. The public OpenAPI document lists route names; the telemetry `404` policy does not hide those names from the document.
...
When a `PersistenceStore` is injected, successful sends record message metadata plus an initial `sent` event. Signed inbound messages are projected before handlers run, using the message's event time rather than receipt time. Delivery/read/failure callbacks update the projected state. Message content is not stored.

Projection failures do not turn a successful Graph send into a failed send. Repeated signed updates retry projection without re-running handlers. Handler failures are still acknowledged; this is not a durable handler-replay queue.
...
Returns a single `MessageRecord` by WhatsApp message id (`wamid.*`). A `null` record returns `404 not_found`; unsafe id segments return `400`.

Both routes return `503 persistence_not_configured` when no `PersistenceStore` is injected. If the injected store throws (outage, lock, corruption), they return `503 persistence_unavailable` instead. A store failure is never misreported as `404 not_found`. A `GET /messages/text` request is treated as the message-id lookup segment `text` only by method — the `POST /messages/text` route takes precedence for POST, and a `GET` to that exact path returns `405` (the `text` segment is reserved by the text-send route).
...
- `401` missing or invalid service bearer token on `/api/*` operator routes
- `404` route not found; telemetry endpoints (`/metrics`, `/status`, `/debug/diagnostics`) also return `404` on a missing or invalid token so their existence stays hidden
...
- `503` persistence failure on a projection route. `persistence_not_configured` means no `PersistenceStore` was injected. `persistence_unavailable` means the injected store threw (outage, lock, corruption); it is never reported as `404 not_found`. `conversation_window_unavailable` covers a throw in the conversation-window lookup specifically.
...
- built-in stores atomically reserve `Idempotency-Key` before Graph is called. Keys are scoped to phone number, WABA, and operation; hashes cover outgoing payloads. Completed sends replay their stored result; a changed payload returns `409 idempotency_conflict`; unresolved claims return `409 idempotency_pending` without sending again;
...
Atomic `claimServiceRequest` / `completeServiceRequest` capabilities are additive: legacy custom stores can still be injected, but keyed sends return `503 persistence_not_atomic` when those methods are absent. Without persistence the service remains stateless, and an idempotency header supplies no deduplication guarantee.

If Graph succeeds but durable completion fails, the service returns the genuine `200` result with `x-wats-persistence: degraded`. Do not resend it. Claims remain unresolved after ambiguous transport failures or crashes; WATS does not automatically release them. Reconcile the outcome before deciding whether a new send is needed.

`POST /api/messages` also accepts templates:

```json
{"type":"template","to":"15550001111","name":"order_confirmation","languageCode":"en_US"}
```

Optional `components` use the SDK template component structure. Templates use the same keyed-send rules. Template approval and recipient eligibility remain Meta-enforced.
