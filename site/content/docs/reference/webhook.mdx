...
<DocMeta status="live-validated" lastReviewed="2026-05-19" />

Simulate an inbound envelope end to end in the [playground](/playground?scenario=webhook-simulator).
...
Meta's webhook delivery may sit behind an optional infrastructure-level mTLS client certificate check. Meta is transitioning outbound client certificate validation to the Meta-owned root `meta-outbound-api-ca-2025-12.pem`. That check is separate from WATS application behavior.
