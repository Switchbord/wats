...
- `phoneNumberId`, `to`, and `callId` must be non-empty strings with no control characters, dot segments, slashes, query/fragment markers, malformed percent encoding, or encoded/double-encoded dot-segments.
