...
description: "The F-7 scoped sub-clients PhoneNumberClient and WABAClient that bind a scope-defining path param at construction."
---
...
`WABAClient` — that sit on top of the F-6 [endpoint registry](/docs/reference/endpoints)
...
This page is a companion to the [Client Reference](/docs/reference/client) and the
[Endpoints Reference](/docs/reference/endpoints). Scoped sub-clients are **thin
...
[errors.md](/docs/reference/errors)) on any violation:
...
- [Endpoints Reference](/docs/reference/endpoints) — the `defineEndpoint`
  primitive that every sub-client method delegates to.
- [Client Reference](/docs/reference/client) — `GraphClient` construction and
  `request` semantics.
- [Errors Reference](/docs/reference/errors) — the F-5 error registry that
