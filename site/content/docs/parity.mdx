...
| Business/admin mutations | profile/commerce/settings updates | `updateBusinessProfile`, `updateCommerceSettings`, `updatePhoneNumberSettings`, Block API, OBA/display-name helpers, and the phone registration lifecycle (`createPhoneNumber`, `requestVerificationCode`, `verifyPhoneNumber`, `registerPhoneNumber`, `deregisterPhoneNumber`, `setTwoStepVerificationPin`). | shape-only. |
| Admin: tokens, QR, catalog CRUD | established | Not built. | planned. |
