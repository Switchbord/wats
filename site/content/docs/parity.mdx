...
description: Per-capability status of WATS against the WhatsApp Cloud API, tagged live-validated, shape-only, or planned.
---

<DocMeta status="active" lastReviewed="2026-06-21" />

Three tags. **live-validated**: exercised against live Meta Graph/webhooks.
**shape-only**: implemented and tested against MockTransport and synthetic
webhooks; Meta has not seen it. **planned**: not built.
...
| OpenTelemetry observability | n/a | `TelemetrySink` seam, `OTEL_ATTR`, `CapturingTelemetrySink`, `NOOP_TELEMETRY_SINK`, `MetricsRegistry`, `createOutboxMetricsReporter`, `/metrics`, `/status`, `/debug/diagnostics`, and a documented OTel adapter. Auto request-scoped span instrumentation is not wired. | shape-only — sink seam and `/metrics` exist; auto request-scoped span instrumentation remains planned. |

Live-validation run log: [campaign log](/docs/parity/live-campaign).
